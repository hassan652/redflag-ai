...
class FsExplorerWorkflow(Workflow):
    ...
    async def start_exploration(
        ...
        async with ctx.store.edit_state() as state:
            ...
            state.compact_context = ev.compact_context
        ...
        try:
            ...
            context_text = format_workspace_context(
                workspace_ctx,
                compact_override=ev.compact_context,
            )
    ...
    async def tool_call_action(
        ...
        agent.configure_task(
            ...
            "REMINDER: When you stop, your final_result MUST include inline "
            "[Source: filename, Section] citations for EVERY factual claim."
        )
    ...
    async def verify_and_finish(
        ...
        if ws_ctx and ws_ctx.structures:
            verification = await verify_answer_async(
                ev.raw_answer,
                ws_ctx,
