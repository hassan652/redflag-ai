...
in the filesystem. Uses MarkItDown (Microsoft, MIT license) for document
conversion — handles PDF, DOCX, PPTX, XLSX, HTML, CSV, and more in a
single dependency with one-line API.
"""
...
SUPPORTED_EXTENSIONS: frozenset[str] = frozenset({
    ".pdf", ".docx", ".pptx", ".xlsx", ".html", ".md", ".csv", ".json", ".xml"
})
...
def _get_cached_or_parse(file_path: str) -> str:
    ...
    Uses MarkItDown for all supported formats.
    ...
    if cache_key not in _DOCUMENT_CACHE:
        ...
        _DOCUMENT_CACHE[cache_key] = result.text_content
