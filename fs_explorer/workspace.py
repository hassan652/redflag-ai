...
1. Parse all documents with Docling (via fs.py)
...
def ingest_folder(
    ...
    2. Parse each with Docling (parallel, with caching)
...
def format_workspace_context(ctx: WorkspaceContext) -> str:
    ...
    for file_path in reading_order:
        if file_path in ctx.structures:
            sections.append(format_structure_for_agent(ctx.structures[file_path]))
            sections.append("")
